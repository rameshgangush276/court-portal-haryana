...
// HELPER: Generate identical code-block for syncing structure
async function generateTableCode() {
...
router.post('/fix-tables', authenticate, requireRole('developer'), async (req, res, next) => {
    try {
        ...
        // Table definitions from single source of truth
        const allTables = require('../../prisma/table-definitions');
...
// ─── 10. POST /api/v1/system/sync-to-code ──────────────────────────────────
// EXPORT structural changes from DB back into the code files (seed.js & system.js)
router.post('/sync-to-code', authenticate, requireRole('developer'), async (req, res, next) => {
    try {
        console.log('📡 [SYNC] Starting Structural Export to Code...');
        const tableArray = await generateTableCode();
        const jsonString = JSON.stringify(tableArray, null, 4);
        ...
        if (fs.existsSync(seedPath)) {
            ...
            if (startIdx !== -1 && endIdx !== -1) {
                const newContent = seedContent.substring(0, startIdx + startMarker.length) 
                                 + jsonString 
                                 + seedContent.substring(endIdx);
                fs.writeFileSync(seedPath, newContent);
                console.log('✅ Updated prisma/seed.js');
            }
        }
        ...
        if (fs.existsSync(systemPath)) {
            ...
            const endMarker = ';\n\n        // Fix sort orders for the 5 correct tables';
            
            const startIdx = sysContent.indexOf(startMarker);
            const endIdx = sysContent.indexOf(endMarker);

            if (startIdx !== -1 && endIdx !== -1) {
                const newContent = sysContent.substring(0, startIdx + startMarker.length) 
                                 + jsonString 
                                 + sysContent.substring(endIdx);
                fs.writeFileSync(systemPath, newContent);
                console.log('✅ Updated server/routes/system.js');
            }
        }
